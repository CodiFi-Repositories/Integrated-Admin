<template>
  <div class="p-4 h-full">
    <div class="card p-4 border rounded bg-white shadow-sm">
      <div class="gap-4 flex">
        ...
        <div class="flex gap-3 items-end">
          ...
          <button type="button" v-if="getEnterprisesLoanData.length"
    ...
    <div class="p-5 bg-white mt-2 overflow-x-auto" v-if="getEnterprisesLoanData?.length">
      <div>
      ...
      <commonPagination v-if="getEnterprisesLoanData && getEnterprisesLoanData.length" v-on:page-chg="getTableData"
        :tot-len="getEnterprisesLoanData.length" :page-date="rowsPerPage" :row-count="rowsCount" />
    </div>

    <div v-else class="p-4 w-full h-full flex items-center justify-center relative font-bold">
...
export default defineComponent({
  ...
  methods: {
    getTableData(data: any) {
      ...
      let lastIndex =
        data.last == "" ? this.getEnterprisesLoanData.length : data.last;
      if (
        this.getEnterprisesLoanData &&
        this.getEnterprisesLoanData.length > 0
      ) {
        this.showData = this.getEnterprisesLoanData.slice(fromIndex, lastIndex);
      } else {
        this.showData = [];
      }
    },
    ...
    getLoanData() {
      if (this.fromDate && this.toDate) {
        ...
        this.$store
          .dispatch("reports/getEnterprisesLoanData", json)
          .finally(() => {
            var data = {
              from: 0,
              last: this.rowsCount,
  ...
  computed: {
    ...mapGetters("reports", ["getEnterprisesLoanData", "getLoader"]),
  },
  unmounted() {
    this.$store.commit("reports/setEnterprisesLoanData", []);
  },
  created() {
    this.setCurrentDatetime();
    this.setPreviousWeekDatetime();
    this.$emit("enterprise_header", this.header);
  },
});
</script>
