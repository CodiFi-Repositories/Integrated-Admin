<template>
  <div class="p-4 h-full">
      ...
      <div class="gap-4 flex">
        <div>
          ...
          <VDatePicker :max-date="new Date().toISOString().slice(0, 10)" v-model="fromDate" is-required
            ...
            <template v-slot="{ togglePopover, inputValue, inputEvents }">
              <div class="flex items-center justify-between min-w-[200px] h-10 border cursor-pointer rounded p-2">
        ...
        <div>
          ...
          <VDatePicker :max-date="new Date().toISOString().slice(0, 10)" :min-date="fromDate" v-model="toDate"
            ...
            input: 'DD-MM-YYYY HH:mm',
            modelValue: 'DD-MM-YYYY  HH:mm',
          }" mode="dateTime" is24hr :disabled-dates="!fromDate">
            <template v-slot="{ togglePopover, inputValue, inputEvents }">
              <div class="flex items-center justify-between min-w-[200px] h-10 cursor-pointer border rounded p-2">
                <input :value="inputValue" placeholder="DD-MM-YYYY" v-on="inputEvents"
                  class="min-w-[150px] h-9 ml-1 text-xs outline-none cursor-pointer" readonly />
                <button type="button"
                  class="flex justify-center mr-2 items-center bg-accent-100 hover:bg-accent-200 text-accent-700"
        ...
        <div class="flex gap-3 items-end">
          ...
          <button type="button" v-if="getSecuritiesLoanData.length"
    ...
    <div class="p-5 bg-white mt-2 overflow-x-auto" v-if="getSecuritiesLoanData?.length">
      <div>
        <table class="border rounded w-full">
          ...
          <tbody class="text-sm">
            <tr class="border-b" v-for="(i, id) in showData" :key="id">
              ...
              <td class="truncate border-r text-center">{{ i.typeOfLoan }}</td>
              <td class="truncate border-r text-center">
                {{ i.portfolioValue }}
              </td>
            </tr>
      ...
      <commonPagination v-if="getSecuritiesLoanData && getSecuritiesLoanData.length" v-on:page-chg="getTableData"
        :tot-len="getSecuritiesLoanData.length" :page-date="rowsPerPage" :row-count="rowsCount" />
    </div>
...
export default defineComponent({
  ...
  setup() {
    ...
    const header = ref([
      ...
      { name: "Type of Loan", csvKey: "typeOfLoan" },
      { name: "Portfolio Value", csvKey: "portfolioValue" },
    ]);
  ...
  methods: {
    getTableData(data: any) {
      ...
      let lastIndex =
        data.last == "" ? this.getSecuritiesLoanData.length : data.last;
      if (this.getSecuritiesLoanData && this.getSecuritiesLoanData.length > 0) {
        this.showData = this.getSecuritiesLoanData.slice(fromIndex, lastIndex);
      } else {
        this.showData = [];
      }
    },
    ...
    getLoanData() {
      ...
      if (this.fromDate && this.toDate) {
        ...
        this.$store
          .dispatch("reports/getSecuritiesLoanData", json)
          .finally(() => {
            var data = {
              from: 0,
              last: this.rowsCount,
  ...
  computed: {
    ...mapGetters("reports", ["getSecuritiesLoanData", "getLoader"]),
  },
  created() {
    this.$emit("security_header", this.header);
    this.setCurrentDatetime();
    this.setPreviousWeekDatetime();
  },
  unmounted() {
    this.$store.commit("reports/setSecuritiesLoanData", []);
  },
});
</script>
