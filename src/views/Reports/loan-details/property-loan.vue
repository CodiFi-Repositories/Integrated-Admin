<template>
  <div class="p-4 h-full">
      ...
      <div class="gap-4 flex">
        ...
        <div>
          ...
          <VDatePicker :max-date="new Date().toISOString().slice(0, 10)" :min-date="fromDate" v-model="toDate"
            ...
            <template v-slot="{ togglePopover, inputValue, inputEvents }">
              <div class="flex items-center justify-between min-w-[200px] cursor-pointer h-10 border rounded p-2">
                <input :value="inputValue" placeholder="DD-MM-YYYY" v-on="inputEvents"
                  class="min-w-[150px] h-9 ml-1 text-xs outline-none cursor-pointer" readonly />
                <button type="button"
                  class="flex justify-center mr-2 items-center bg-accent-100 hover:bg-accent-200 text-accent-700"
        ...
        <div class="flex gap-3 items-end">
          ...
          <button type="button" v-if="getPropertyLoanData.length"
    ...
    <div class="p-5 bg-white mt-2 overflow-x-auto" v-if="getPropertyLoanData?.length">
      <div>
      ...
      <commonPagination v-if="getPropertyLoanData && getPropertyLoanData.length" v-on:page-chg="getTableData"
        :tot-len="getPropertyLoanData.length" :page-date="rowsPerPage" :row-count="rowsCount" />
    </div>
...
export default defineComponent({
  ...
  methods: {
    getTableData(data: any) {
      ...
      let lastIndex =
        data.last == "" ? this.getPropertyLoanData.length : data.last;
      if (this.getPropertyLoanData && this.getPropertyLoanData.length > 0) {
        this.showData = this.getPropertyLoanData.slice(fromIndex, lastIndex);
      } else {
        this.showData = [];
      }
    },
    ...
    getLoanData() {
      if (this.fromDate && this.toDate) {
        ...
        this.$store
          .dispatch("reports/getPropertyLoanData", json)
          .finally(() => {
            var data = {
              from: 0,
              last: this.rowsCount,
  ...
  computed: {
    ...mapGetters("reports", ["getPropertyLoanData", "getLoader"]),
  },
  created() {
    this.$emit("property_header", this.header);
    this.setCurrentDatetime();
    this.setPreviousWeekDatetime();
  },
  unmounted() {
    this.$store.commit("reports/setPropertyLoanData", []);
  },
});
</script>
