<template>
  <div>
    <TransitionRoot appear :show="paymentResponseDialog" as="template">
          ...
          >
            ...
            >
              ...
              >
                      ...
                      <div class="grid grid-cols-2">
                        <div class="flex pb-4">
                          ...
                          <div class="text-xs text-gray-900">
                            {{
                              paymentResponseRowData?.user_id ||
                              paymentResponseRowData?.userId
                            }}
                          </div>
                        </div>
                      </div>
                ...
                <div
                  class="overflow-y-auto max-h-[300px] h-[300px] scrollModifty"
                >
                  <div class="flex justify-between items-center mt-4 text-xs">
                    <p>Request Body:</p>
                    <button
                      class="border px-4 h-8 rounded text-xs mr-2"
...
export default {
  ...
  methods: {
    ...
    copyRequestBody() {
      this.copyText = false;
      if (this.paymentResponseRowData.request?.length > 0) {
        navigator.clipboard.writeText(this.paymentResponseRowData.request);
      }
    ...
    copyResponseBody() {
      ...
      if (this.paymentResponseRowData.response?.length > 0) {
        navigator.clipboard.writeText(this.paymentResponseRowData.response);
      }
    ...
    getValue(type) {
      if (type == "req") {
        return this.isJsonString(this.paymentResponseRowData?.request);
      } else {
        return this.isJsonString(this.paymentResponseRowData?.response);
      }
    },
  ...
  computed: {
    ...mapState("reports", ["paymentResponseDialog", "paymentResponseRowData"]),
  },
