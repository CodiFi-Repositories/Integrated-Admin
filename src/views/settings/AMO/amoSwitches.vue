  ...
  <section>
    <div class="pt-5 px-5 h-full">
      <div class="bg-white p-4 border rounded">
        <h1 class="pb-5">AMO Controls</h1>
        <div class="flex flex-col gap-2">
          <div class="flex items-center gap-3">
            <p class="text-[14px] min-w-[135px]">AMO Always On</p>
            <Switch
              @click="setIsUpdate()"
              v-model="isAlwaysOn"
          ...
          <div class="flex items-center gap-3">
            <p class="text-[14px] min-w-[135px]">AMO Always Off</p>
            <Switch
              @click="setIsUpdate()"
              v-model="isAlwaysOff"
          ...
          <div class="flex items-center gap-3">
            <p class="text-[14px] min-w-[135px]">AMO Check Timings</p>
            <Switch
              @click="setIsUpdate()"
...
export default defineComponent({
  ...
  computed: {
    ...
    isAlwaysOn: {
      ...
      set(val: any) {
        this.$store.commit("amoControls/setIsAlwaysOn", val);
        if (val) {
          this.$store.commit("amoControls/setIsAlwaysOff", !val);
          this.$store.commit("amoControls/setIsCheckTimings", !val);
        } else {
          this.$store.commit("amoControls/setIsAlwaysOff", !val);
        }
      },
  ...
  mounted() {
    this.$store.dispatch("amoControls/getAmoControls");
  },
  methods: {
    setIsUpdate() {
      this.isUpdate = true;
    ...
    updateAmoSwitches() {
      this.$store.dispatch("amoControls/updateAmoControls");
    },
  },
});
</script>
